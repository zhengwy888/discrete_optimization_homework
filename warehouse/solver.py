...
def solveIt(inputData):
    ...
    Printsolution = True
    Bighole = False # failed
    Customerfirst = False # select lowest cost warehouse for all customers, failed

    Houseswap = False # try open and close warehouses, redistribute customers along the way
    Streetclean = False #during housewap, fill up new warehouses as much as possible
    Juggle = True # swapping customers between two opened warehouses
    
    ## greedy
    Simplerank = False # good first guess
    Rankpeek = False # rank warehouses based on customer priorities
    Aggrepeek = True # looking at aggregated cost of warehouses
    Aggrerank = True # use aggregated cost to open warehouses
    Aggrefactorial = True # from aggrerank, include one or more warehouses that are not opened. 
    ...
    if Rankpeek:
        ...
        # for whtemp in whRanksorted:
            # print 'warehouse ' + str(whtemp[0]) + ' likes ' + str(whtemp[1])
        # return
        
        
    if Simplerank:
        whPricerank = whRanksorted
        ...
        bestCost = solutionCost(solution, whUsed, customerCosts, warehouses)
        if not Printsolution:
            print 'Simplerank cost is ' + str(bestCost) + ' opened ' + str(whUsed.count()) 
        
    if Aggrepeek:
        aggreCost = [0] * warehouseCount
        for wh in range(0, warehouseCount):
            aggreCost[wh] += warehouses[wh][1]
            for c in range(0, customerCount):
                aggreCost[wh] += customerCosts[c][wh]
        aggreCostsorted = sorted(enumerate(aggreCost), key=lambda item:item[1])
   
    if Aggrerank:
        # largest customer first. place them
        cSizesorted = sorted(enumerate(customerSizes), key = lambda item: item[1], reverse=True )
        #whCustomers = [[] for x in xrange(warehouseCount)] 
        whUsed = bitarray(warehouseCount)
        bestCost = 99999999999999999
        for warehouseopened in range(1, warehouseCount):
            whUsed.setall(False)
            capRemain = [w[0] for w in warehouses]
            solution = [-1] * customerCount
            customerPlaced = 0
            # just open a fix number of warehouses
            # for wl_50_1 try 15
            for i in range(0, warehouseopened):
                wh = aggreCostsorted[i][0]
                whUsed[wh] = True
            for ctemp in cSizesorted:
                c = ctemp[0]
                size = ctemp[1]
                whsubrank = cCostsSorted[c]
                for wh in whsubrank:
                    if (whUsed[wh] and size <= capRemain[wh]):
                            #addCustomer(wh, c, whCustomers, whUsed, capRemain, customerSizes)
                            addCustomersol(wh, c, solution, whUsed, capRemain, customerSizes)
                            solution[c] = wh
                            customerPlaced += 1
            ...
            if cost <= bestCost and customerPlaced == customerCount:
                bestSolution = list(solution)
                bestCost = cost
                bestwhUsed = whUsed.copy()
        
        # for whtemp in aggreCostsorted:
            # if customerPlaced == customerCount:
                # break
            # wh = whtemp[0]
            # for ctemp in cSizesorted:
        ...
        #bestCost = solutionCost(solution, whUsed, customerCosts, warehouses)
        warehouseopened = bestwhUsed.count()
        if not Printsolution:
            print 'Aggrerank cost is ' + str(bestCost) + ' opened ' + str(bestwhUsed.count()) 
        #print 'not place customer ' + str(bestSolution.count(-1))
        obj = bestCost
        solution = bestSolution
        # return
    
    if Aggrefactorial:
        numToSkip = warehouseopened
        whSkipped = range(0, numToSkip)
        running = True
        while running:
            #print 'warehouse skipped ' + ','.join(map(str, whSkipped))
            whUsed.setall(False)
            capRemain = [w[0] for w in warehouses]
            solution = [-1] * customerCount
            customerPlaced = 0
            whLimit = warehouseopened+numToSkip
            if whLimit > warehouseCount:
                whLimit = warehouseCount
            for i in range(0, whLimit):
                try:
                    found = whSkipped.index(i)
                except ValueError:
                    found = -1
                if ( found != -1 ):
                    continue
            ...
            clump = 0
            for k in range(0, numToSkip):
                if whSkipped[numToSkip - k - 1] == whLimit - k - 1:
                    clump += 1
                else:
                    break
            if (clump > 0) :
                if ( clump == numToSkip ):
                    running = False
                    continue
                gap = whSkipped[numToSkip - clump] - whSkipped[numToSkip-1 - clump] - 2
                for k in range(0, clump):
                    whSkipped[numToSkip - 1 - k] -= gap
                whSkipped[numToSkip-1 - clump] += 1
            else:
                whSkipped[numToSkip-1] += 1
        if not Printsolution:
            print 'Aggrerandom improved cost to ' + str(bestCost) 

        
    if ( Logger ) :
        fh = open('calculation.csv', 'w')
        fh.write("time, bestcost, cost, warehousecount\n")
        fh.write(str(time.time() - startts)+','+str(bestCost)+ ',' + str(cost) + ',' + str(whUsed.count(True))+ "\n")
    ...
    if Houseswap:    
        ...
        operation = 'open'
        prevCost = bestCost
        leastUsed = warehouseCount
        margin = int(0.15 * warehouseCount)
        running = True
        duration = 0.5
        loopstartts = time.time()
        while running:
            if ( time.time() - loopstartts > duration * 60 ):
                running = False
            #print 'operation is ' + str(operation)
            if ( operation == 'open' ):
                #do open step
                if whUsed.count(True) > (leastUsed + margin):
                    operation = 'close'
                    continue
                for i in range(0, warehouseCount+1):
                    if ( i == warehouseCount ):
            ...
            if ( cost < bestCost ) :
                print 'current best is ' + str(bestCost)
                bestCost = cost
                bestSolution = tuple(solution)
                #exit()
                whUsedcount = whUsed.count(True)
                if whUsedcount < leastUsed:
                    leastUsed = whUsed.count(True)
            if ( Logger ):
                fh.write(str(time.time() - startts)+','+str(bestCost)+ ',' + str(cost) + ',' + str(whUsed.count(True))+ "\n")
    ...
    if Juggle:
        ...
        while running:
            if ( time.time() - loopstartts > duration * 60 ) :
                running = False
            whOne = random.randint(0, warehouseCount - 1)
            while whUsed[whOne] == False:
                whOne = random.randint(0, warehouseCount - 1)
            whTwo = whOne
            while whTwo == whOne or whUsed[whTwo] == False:
                whTwo = random.randint(0, warehouseCount - 1) 
            cOne = getCustomers(solution, whOne)
            cTwo = getCustomers(solution, whTwo)
            beneOneTwo = [(c, (customerCosts[c][whTwo] - customerCosts[c][whOne])) for c in cOne]
            beneOneTwoSorted = sorted(beneOneTwo, key = lambda item:item[1])
            beneTwoOne = [(c, (customerCosts[c][whOne] - customerCosts[c][whTwo])) for c in cTwo]
            beneTwoOneSorted = sorted(beneTwoOne, key = lambda item:item[1])
            if ( beneOneTwoSorted[0][1] > 0 and beneTwoOneSorted[0][1] > 0):
                #swapping hurts
                #print 'swapping will hurt'
                continue
            exchangeOne = []
            sizechange = [0] * 2
            exchangeTwo = []
            ...
            exidxone = 0
            exidxtwo = 0
            if ( beneOneTwoSorted[0][1] < beneTwoOneSorted[0][1] ):
                #while beneOneTwoSorted[exidxone][1] < 0 and exidxone < len(cOne):
                exchangeOne.append(beneOneTwoSorted[exidxone][0])
                sizechange[0] += customerSizes[beneOneTwoSorted[exidxone][0]]
                exidxtwo += 1
            else:
                #while beneOneTwoSorted[exidxtwo][1] < 0 and exidxtwo < len(cOne):
                exchangeOne.append(beneTwoOneSorted[exidxtwo][0])
                sizechange[1] += customerSizes[beneTwoOneSorted[exidxtwo][0]]
                exidxtwo += 1
            sizetest = []
            sizetest.append((capRemain[whOne] + sizechange[0]) > sizechange[1])
            sizetest.append((capRemain[whTwo] + sizechange[1]) > sizechange[0])
            swapright = True
            while sizetest[0] != True or sizetest[1] != True:
                if (sizetest[0] == sizetest[1]):
                    #both false
                    swapright = False
                    break
                elif ( sizetest[0] != True ):
                    # two gave out a large item
                    if (exidxone >= len(cOne)):
                        swapright = False
                        break
                    exchangeOne.append(beneOneTwoSorted[exidxone][0])
                    sizechange[0] += customerSizes[beneOneTwoSorted[exidxone][0]]
                    ...
                    if (exidxtwo >= len(cTwo)):
                        swapright = False
                        break
                    # one gave out a large item
                    exchangeTwo.append(beneTwoOneSorted[exidxtwo][0])
                    sizechange[1] += customerSizes[beneTwoOneSorted[exidxtwo][0]]
                    exidxtwo += 1
                sizetest[0] = ((capRemain[whOne] + sizechange[0]) > sizechange[1])
                sizetest[1] = ((capRemain[whTwo] + sizechange[1]) > sizechange[0])
            if swapright:
                # test value
                value = 0
                for idx in range(0, exidxone):
                    value += beneOneTwoSorted[idx][1]
                for idx in range(0, exidxtwo):
                    value += beneTwoOneSorted[idx][1]
                if value < 0:
                    for c in exchangeOne:
                        removeCustomersol(whOne, c, solution, whUsed, capRemain, customerSizes)
                        addCustomersol(whTwo, c, solution, whUsed, capRemain, customerSizes)
                    for c in exchangeTwo:
                        removeCustomersol(whTwo, c, solution, whUsed, capRemain, customerSizes)
                        addCustomersol(whOne, c, solution, whUsed, capRemain, customerSizes)
                    cost += value
                    if not Printsolution:
                        print 'reduced ' + str(value) + ' to ' + str(cost)
                else:
                    #print 'swap is not constructive'
                    pass
            else:
                #print 'swap not alright, try again'
                pass
        
        bestCost = cost
        bestSolution = solution
        #print 'solution cost ' + str(solutionCost(solution, whUsed, customerCosts, warehouses))
    ...
    outputData = str(bestCost) + ' ' + str(0) + '\n'
    outputData += ' '.join(map(str, bestSolution))
